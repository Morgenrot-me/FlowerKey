...
<template>
  <div class="space-y-3">
    ...
    <p class="text-[11px] text-gray-500 dark:text-gray-400 leading-relaxed">花钥使用“记忆密码 + 身份密语 + 区分代号”离线确定性生成密码。换到空设备时，三项输入完全一致即可重建同一密码。</p>
    <p class="text-[11px] text-orange-600 dark:text-orange-400 flex items-center gap-1.5"><AppIcon name="alert" :size="12" class-name="shrink-0" /> 记忆密码和身份密语共同决定全部生成密码。设置后不得修改，也不要告知他人。</p>
    <div class="rounded-xl border border-blue-200/70 bg-blue-50/70 px-3 py-2 text-[11px] text-blue-700 dark:border-blue-800/70 dark:bg-blue-900/20 dark:text-blue-200">
    ...
    <input
      ...
      class="w-full px-3 py-2 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-100"
    />
    <p class="text-[11px] text-gray-400 dark:text-gray-500">记忆密码决定所有生成密码的结果，输入有误将导致生成不同密码，确认输入以保证一致性。</p>

    <div class="space-y-1.5">
      <p class="text-[11px] text-gray-600 dark:text-gray-300 font-medium">身份密语</p>
      <p class="text-[11px] text-gray-500 dark:text-gray-400 leading-relaxed">创建一条只有你知道、多年后仍能完整写出的私人身份句。它区分大小写，不要求数字、大写或符号；不要使用公开昵称、生日、学校、宠物名或其他服务的密码。</p>
      <input
      ...
      <input
        ...
        class="w-full px-3 py-2 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-100"
      />
      <p class="text-[11px] text-gray-400 dark:text-gray-500">身份密语区分大小写、空格和标点；换设备时必须完整输入同一内容。</p>
    </div>
