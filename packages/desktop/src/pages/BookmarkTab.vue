  ...
  <div class="h-full flex flex-col">
      ...
      <div v-for="e in store.filtered" :key="e.id" class="px-4 py-3 flex items-center gap-3">
        <div class="flex-1 min-w-0 cursor-pointer hover:bg-gray-50 -mx-4 px-4 py-2 -my-2 rounded transition-colors" @click="openEdit(e)">
          ...
          <div v-if="e.tags?.length" class="flex gap-1 mt-1">
            <span v-for="t in e.tags" :key="t" class="px-1 bg-gray-100 rounded text-[10px]">{{ t }}</span>
          </div>
        </div>
        <button @click="handleDelete(e.id)" class="text-red-400 text-sm px-2 hover:text-red-600">删除</button>
      </div>
    ...
    <div v-if="showForm" class="absolute inset-0 bg-white flex flex-col" style="padding-top: env(safe-area-inset-top)">
      <div class="px-4 py-3 border-b flex items-center gap-3">
        <button @click="showForm = false; editingId = ''; tagInput = ''; form = { title: '', url: '', tags: [], description: '' }" class="text-blue-500">取消</button>
        <span class="flex-1 text-center font-medium">{{ editingId ? '编辑书签' : '新建书签' }}</span>
      ...
      <div class="flex-1 px-4 py-4 flex flex-col gap-3 overflow-y-auto">
        ...
        <!-- 标签 -->
        <div>
          <input v-model="tagInput" placeholder="添加标签，回车确认" @keydown.enter.prevent="addTag" class="w-full px-3 py-3 border rounded-xl text-base outline-none focus:border-blue-400" />
        ...
        <textarea v-model="form.description" placeholder="描述（可选）" rows="2" class="w-full px-3 py-3 border rounded-xl text-base outline-none focus:border-blue-400 resize-none" />
      </div>
    </div>
