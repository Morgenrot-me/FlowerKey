  ...
  <div class="h-full overflow-y-auto px-4 py-4 flex flex-col gap-4">
    ...
    <div class="bg-white rounded-xl">
      ...
      <div v-if="showSecurity" class="px-4 pb-4 flex flex-col gap-2 text-xs text-gray-500 leading-relaxed">
        ...
        <p class="text-gray-600 font-medium pt-1">加密算法</p>
        <p>AES-256-GCM 是目前最主流的对称加密标准，1Password、Bitwarden 等主流密码管理工具均采用此算法。花钥用它加密区分代号等敏感字段——但<span class="text-gray-700 font-medium">单独的区分代号无法算出密码</span>，最终密码由"区分代号 + 你的记忆密码"共同决定。记忆密码只存在于你的脑中，从不上传、从不存储，密码的最终所有权永远属于你。</p>
      </div>
    </div>
