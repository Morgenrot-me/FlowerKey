...
| `gradle.properties` | Gradle 运行参数、本地 Java 路径、AndroidX 与构建缓存配置。 |
...
- Android 构建要求 Java 21 toolchain，当前 `gradle.properties` 使用 Android Studio JBR：`D:\APP\C\Android\jbr`。
...
- Java 路径是本机构建约定，迁移到新机器时需确认该路径存在且版本为 Java 21。
